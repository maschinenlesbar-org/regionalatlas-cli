...
| **Indikator (indicator)** | `indicators`, `query <code>` | A single measurable variable, identified by a **code** (e.g. `AI002-1-5`). There are **71** (on 2026-09-26; the catalogue changes). |
...
| **typ / geo level** | `--level` | The geographic aggregation level. `land`=1 (Bundesländer, 16), `regierungsbezirk`/`rb`=2 (38 rows in recent years), `kreis`/`landkreis`=3 (400 rows in recent years), `gemeinde`=5 (~11 000 rows). Row counts follow the boundaries of the reporting year: for 2000, `rb` has 40 rows and `kreis` 440, among them two Kreise named Hannover (`03201`, `03253`) from before the Region Hannover. Every level covers **all of Germany** — see *level fill-in*. Not every indicator is published at every level (`AIGG-01` only per Land, `AI005` not per Gemeinde): the catalogue's `geom_levels` say which levels have figures in a year, and `query` refuses another level (exit 2, naming the published ones) instead of returning a row per region with every value `null`. |
| **level fill-in** | `ags` length | Where a finer unit does not exist, a level fills in with the next coarser one, so each level is a complete, non-overlapping cover of Germany and `ags` length varies within it. `regierungsbezirk` returns 38 rows for recent years: the **29** actual Regierungsbezirke (3-digit AGS) plus the **9 Bundesländer that have none** (2-digit: Schleswig-Holstein, Hamburg, Bremen, Saarland, Berlin, Brandenburg, Mecklenburg-Vorpommern, Sachsen-Anhalt, Thüringen). `kreis` and `gemeinde` carry Berlin (`11`) and Hamburg (`02`) at 2 digits; `gemeinde` also carries 104 kreisfreie Städte at their 5-digit Kreis key. So "the 38 Regierungsbezirke" is wrong, but summing or mapping a level is safe. |
