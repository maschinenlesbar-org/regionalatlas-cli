...
| **typ / geo level** | `--level` | The geographic aggregation level. `land`=1 (Bundesländer, 16), `regierungsbezirk`/`rb`=2 (38 rows), `kreis`/`landkreis`=3 (400 rows), `gemeinde`=5 (~11 000 rows). Every level covers **all of Germany** — see *level fill-in*. Not every indicator is published at every level (`AIGG-01` only per Land, `AI005` not per Gemeinde): the catalogue's `geom_levels` say which levels have figures in a year, and `query` refuses another level (exit 2, naming the published ones) instead of returning a row per region with every value `null`. |
