...
| **typ / Gebietsebene** | `--level` | Die räumliche Aggregationsebene. `land`=1 (Bundesländer, 16), `regierungsbezirk`/`rb`=2 (38 Zeilen), `kreis`/`landkreis`=3 (400 Zeilen), `gemeinde`=5 (ca. 11 000 Zeilen). Jede Ebene deckt **ganz Deutschland** ab – siehe *Auffüllen der Ebene*. Nicht jeder Indikator wird auf jeder Ebene veröffentlicht (`AIGG-01` nur je Land, `AI005` nicht je Gemeinde): Die `geom_levels` des Katalogs sagen, auf welchen Ebenen ein Jahr Werte hat, und `query` lehnt eine andere Ebene ab (Exit 2, mit den veröffentlichten Ebenen), statt je Region eine Zeile nur mit `null`-Werten zu liefern. |
